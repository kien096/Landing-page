    ...
    <section id="trust" className="py-48 relative overflow-hidden bg-black text-white">
          ...
          {scrollingReviews.map((review, idx) => (
            ...
            >
              <div className="flex justify-between items-start mb-8 sm:mb-12 lg:mb-16">
                ...
                <Quote size={40} className="sm:w-[60px] sm:h-[60px] text-white/[0.02] group-hover:text-brand-500/10 transition-all transform group-hover:-rotate-12 duration-1000" fill="currentColor" />
              </div>

              <p className="text-slate-300 text-lg sm:text-xl lg:text-2xl leading-relaxed mb-8 sm:mb-12 lg:mb-16 font-medium italic tracking-tight">
                "{review.content}"
              </p>

              <div className="flex items-center gap-4 sm:gap-6 lg:gap-8 mt-auto">
                <div className={`w-14 h-14 sm:w-16 sm:h-16 lg:w-20 lg:h-20 ${review.color} rounded-[1.2rem] sm:rounded-[1.5rem] lg:rounded-[2rem] flex items-center justify-center text-white font-black text-lg sm:text-xl lg:text-2xl shadow-xl transform transition-transform shrink-0`}>
                  {review.initials}
                </div>
                <div className="space-y-1">
                  <div className="font-black text-white text-xl sm:text-2xl tracking-tighter">{review.name}</div>
                  <div className="flex flex-col">
                    <span className="text-[9px] sm:text-[10px] font-black text-slate-500 uppercase tracking-[0.25em] sm:tracking-[0.4em] mb-1 leading-none">{review.role}</span>
      ...
      <div className="container mx-auto px-4 sm:px-6 mt-16 sm:mt-24 lg:mt-32">
         <div className="flex flex-wrap justify-center items-center gap-8 sm:gap-12 lg:gap-36 opacity-10 grayscale Contrast-200 hover:opacity-100 hover:grayscale-0 transition-all duration-2000">
           <span className="text-2xl sm:text-3xl lg:text-4xl font-black text-white italic tracking-tighter uppercase whitespace-nowrap">EuroPlast</span>
           <span className="text-2xl sm:text-3xl lg:text-4xl font-black text-white italic tracking-tighter uppercase whitespace-nowrap">Abbey Vietnam</span>
           <span className="text-2xl sm:text-3xl lg:text-4xl font-black text-white italic tracking-tighter uppercase whitespace-nowrap">BanhMyBa</span>
           <span className="text-2xl sm:text-3xl lg:text-4xl font-black text-white italic tracking-tighter uppercase whitespace-nowrap">POLYFILL</span>
         </div>
      </div>
    </section>
  );
